...
fun upperCircleFunction(c: MyCircleData, x: Float): Float {
...
fun lowerCircleFunction(c: MyCircleData, x: Float): Float {
    if (round(c.x - c.r) > x || round(c.x + c.r) < x) {
//        throw Exception("The requested point lies outside of the circle: $x  c.x: ${c.x}  c.r: ${c.r}")
        return 0f
    }
...
//fun checkAll(rect:Rectangle, circles: ArrayList<MyCircleData>): MutableList<Float>{
...
//}
//
//fun checkCircle(c:MyCircleData,x:Float,y:Float):Boolean{
